...
describe('processRequest', () => {
  ...
  it('respects X-Forwarded-* headers', async () => {
    const response = await makeRequest('/mybundle.bundle?runModule=true', {
      headers: {
        host: 'localhost:8081',
        'x-forwarded-proto': 'https',
        'x-forwarded-host': 'proxyhost:10101',
      },
    });
    ...
    expect(response.body).toEqual(
      [
        ...
        '__d(function() {foo();},1,[],"foo.js");',
        'require(0);',
        '//# sourceMappingURL=https://proxyhost:10101/mybundle.map?runModule=true',
        '//# sourceURL=https://proxyhost:10101/mybundle.bundle?runModule=true',
      ].join('\n'),
    );
  });

  it('respects Forwarded header', async () => {
    const response = await makeRequest('/mybundle.bundle?runModule=true', {
      headers: {
        host: 'localhost:8081',
        forwarded: 'by=secret;proto=https;host=proxyhost:10101',
      },
    });
